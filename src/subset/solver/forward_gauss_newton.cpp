...
// Forward-compositional warp update:  W(x;p_new) = W(x;p) �� W(x;��p).
...
) const
{
    ...
    for (int iteration = 0; iteration < config_.max_iterations; ++iteration) {
        ...
        // Forward-compositional update  W(x;p_new) = W(x;p) �� W(x;��p)
...
// W(x;p_new) = W(x;p) �� W(x;��p), expanded up to second order in (lx,ly) and
