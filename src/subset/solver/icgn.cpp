...
// Bai et al. (2017) Eq (20–22).  The matrix maps the augmented
// homogeneous coordinate  [1, x, y, x², xy, y²]ᵀ  to
// [x', y', 1, x'², x'y', y'²]ᵀ  up to  o(x²+y²).
//
    ...
    const Eigen::Matrix<double, 12, 1>& p)
{
    ...
    // ---- Row 0:  x' = u + (1+ux)·x + uy·y + ½uxx·x² + uxy·xy + ½uyy·y² ----
    G(0, 0) = u;
    G(0, 1) = 1.0 + ux;
    G(0, 2) = uy;
    G(0, 3) = 0.5 * uxx;
    ...
    // ---- Row 1:  y' = v + vx·x + (1+vy)·y + ½vxx·x² + vxy·xy + ½vyy·y² ----
    G(1, 0) = v;
    G(1, 1) = vx;
    G(1, 2) = 1.0 + vy;
    G(1, 3) = 0.5 * vxx;
    ...
    // ---- Row 3:  x'² = (x + ū)²  (ū = u-displacement) ------------------------
    // Derived from:  x'² = x² + 2x·ū + ū²,  keeping terms ≤ O(x², y²).
    G(3, 0) = u * u;
    G(3, 1) = 2.0 * u * (1.0 + ux);
    G(3, 2) = 2.0 * u * uy;
    ...
    // ---- Row 4:  x'y' = (x + ū)(y + v̄)  (v̄ = v-displacement) ----------------
    // Derived from:  x'y' = xy + x·v̄ + y·ū + ū·v̄,  keeping terms ≤ O(x², y²).
    G(4, 0) = u * v;
    G(4, 1) = v + u * vx + v * ux;
    G(4, 2) = u + u * vy + v * uy;
    ...
    // ---- Row 5:  y'² = (y + v̄)² -----------------------------------------------
    // Derived from:  y'² = y² + 2y·v̄ + v̄²,  keeping terms ≤ O(x², y²).
    G(5, 0) = v * v;
    G(5, 1) = 2.0 * v * vx;
    G(5, 2) = 2.0 * v * (1.0 + vy);
    G(5, 3) = vx * vx + v * vxx;
    ...
    const Eigen::Matrix<double, 6, 6>& G)
{
    ...
    // From Row 0:  x' = u + (1+ux)·x + uy·y + ½uxx·x² + uxy·xy + ½uyy·y²
    p(0) = G(0, 0);                  // u
    p(2) = G(0, 1) - 1.0;            // ux
    p(3) = G(0, 2);                  // uy
    p(6) = 2.0 * G(0, 3);            // uxx
    ...
    // From Row 1:  y' = v + vx·x + (1+vy)·y + ½vxx·x² + vxy·xy + ½vyy·y²
    p(1)  = G(1, 0);                 // v
    p(4)  = G(1, 1);                 // vx
    p(5)  = G(1, 2) - 1.0;           // vy
    p(9)  = 2.0 * G(1, 3);           // vxx
...
// Optics and Lasers in Engineering, 90, 48–58.  Eq (23) + Appendix A.
//
...
//
// where G'(p) maps INPUT  v = [1, x, y,  x²,  xy,  y² ]ᵀ
//                    to OUTPUT w = [x', y', 1, x'², x'y', y'²]ᵀ.
// P⁻¹ is the permutation that restores the w-convention (v → w)
// so that G'(Δp)⁻¹ : w → v can be composed with G'(p_old) : v → w.
//
    ...
    const Eigen::Matrix<double, 12, 1>& delta)
{
    ...
    // The paper proves G'(Δp) is invertible for physically realisable
    ...
    // Compose:  G'(p_new) = G'(p_old) · [G'(Δp)]⁻¹ · P⁻¹
    //
    // G'(·) maps input convention  v = [1, x, y, x², xy, y²]ᵀ
    //                     to output w = [x', y', 1, x'², x'y', y'²]ᵀ.
    // G'(Δp)⁻¹ : w → v.  To feed its output into G'(p_old) : v → w
    // we must first restore the w-convention with the permutation
    // P⁻¹ that sends v → w (i.e. cycles [1,x,y] → [x,y,1]).
...
) const
{
    ...
    // --- Hessian: H = J^T * J (no 2/σ² scaling) ---
    Eigen::Matrix<double, 6, 6> hessian = Eigen::Matrix<double, 6, 6>::Zero();
    for (const auto& sample : samples) {
        hessian += sample.steepest_descent * sample.steepest_descent.transpose();
    }
    ...
    for (int iteration = 0; iteration < config_.max_iterations; ++iteration) {
        ...
        // --- Gradient: Σ[(ref - def) * J]  (no 2/σ scaling) ---
        Eigen::Matrix<double, 6, 1> gradient = Eigen::Matrix<double, 6, 1>::Zero();
        ssd_corrcoef = 0.0;
        for (std::size_t i = 0; i < samples.size(); ++i) {
...
) const
{
    ...
    for (int iteration = 0; iteration < config_.max_iterations; ++iteration) {
        ...
        // --- Gradient: (2/σ)·Σ[normalized_diff · J] ---
...
) const
{
    ...
    // --- Hessian: 12×12, H = J^T * J (no 2/σ² scaling for SSD) ---
    Eigen::Matrix<double, 12, 12> hessian = Eigen::Matrix<double, 12, 12>::Zero();
    for (const auto& sample : samples) {
        hessian += sample.steepest_descent * sample.steepest_descent.transpose();
    }
    ...
    for (int iteration = 0; iteration < config_.max_iterations; ++iteration) {
        ...
        // --- Gradient: Σ[(ref - def) * J] (no 2/σ scaling for SSD) ---
        Eigen::Matrix<double, 12, 1> gradient = Eigen::Matrix<double, 12, 1>::Zero();
        ssd_corrcoef = 0.0;
        for (std::size_t i = 0; i < samples.size(); ++i) {
            const double residual = samples[i].reference_value - deformed_values[i];
