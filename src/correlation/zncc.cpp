/**
 * @file zncc.cpp
 * @brief Zero-normalized cross-correlation implementation.
 *
 * Responsibilities:
 * - Validate correlation vector inputs.
 * - Evaluate normalized cross-correlation after removing mean intensity and contrast scale.
 ...
 *
 * Outputs:
 * - Weighted ZNCC score where higher is better.
 *
 * Dependencies:
 * - Corresponding public header plus Eigen/OpenCV-ready module boundaries.
 *
 * TODO:
 * - Add weighted/masked ZNCC variants.
 * - Add batch evaluation helpers for initialization modules.
 */
...
)
{
    ...
    if (norm <= kVarianceEpsilon) {
        throw std::invalid_argument("ZNCC requires nonzero subset intensity variance.");
    }
...
double ZNCCCorrelation::evaluate(
    const Eigen::VectorXd& reference,
    const Eigen::VectorXd& deformed
) const
{
...
) const
{
    ...
    const double score = (weights.array() * normalized_reference.array() * normalized_deformed.array()).sum();
