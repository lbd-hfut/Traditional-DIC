...
static dic::SubsetConfig config_from_dict(py::dict d) {
    ...
    if (d.contains("shape_function")) {
        ...
        if (s.contains("order")) {
            // YAML order may be int (2) or string ("second_order"), so go
            // through py::str before matching (mirrors yaml_parser.cpp's
            // parse_shape_function, which is file-local there).
