...
TEST(Triangulation, RecoversKnownPointFromThreeViews)
{
    ...
    const auto result = dic::triangulate_points_checked(observations, cameras);
...
TEST(Triangulation, RejectsPointBehindCamera)
{
    const std::vector<dic::CameraModel> cameras = {
        make_camera({0.0, 0.0, 0.0}),
        make_camera({120.0, 0.0, 0.0}),
...
TEST(Triangulation, RecoversKnownPointWithDistortedObservations)
{
    std::vector<dic::CameraModel> cameras = {
        make_camera({0.0, 0.0, 0.0}),
        make_camera({120.0, 0.0, 0.0}),
        make_camera({0.0, 90.0, 0.0}),
    };
    for (auto& camera : cameras) {
        camera.distortion = {0.04, -0.01, 0.001, -0.0007, 0.002};
    }

    const Eigen::Vector3d expected(18.0, -12.0, 920.0);
    std::vector<Eigen::Vector2d> observations;
    for (const auto& camera : cameras) {
        observations.push_back(dic::project_point(expected, camera));
    }
    ...
    ASSERT_TRUE(result.valid);
    EXPECT_NEAR((result.point - expected).norm(), 0.0, 1e-5);
    EXPECT_NEAR(result.max_reprojection_error, 0.0, 1e-5);
}
