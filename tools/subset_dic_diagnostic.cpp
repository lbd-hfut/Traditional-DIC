...
{
    if (argc != 6) {
        std::cerr << "Usage: subset_dic_diagnostic <reference> <deformed> <roi> <out_csv> <config_yaml>\n";
        return EXIT_FAILURE;
    }
    ...
    try {
        ...
        std::cout << "total_points=" << results.size() << '\n'
                  << "valid_points=" << valid_count << '\n'
                  << "invalid_points=" << (results.size() - valid_count) << '\n';
        return EXIT_SUCCESS;
    } catch (const std::exception& ex) {
        std::cerr << "subset_dic_diagnostic failed: " << ex.what() << '\n';
        return EXIT_FAILURE;
    }
}
