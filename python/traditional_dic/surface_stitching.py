...
def write_stitch_visualizations(result: StitchResult, output_dir, *, max_faces: int | None = 70000) -> None:
    ...
    valid_faces = (
        (faces >= 0).all(axis=1)
        & (faces < len(reference)).all(axis=1)
        & np.all(np.isfinite(reference[faces]), axis=(1, 2))
        & np.all(np.isfinite(deformed[faces]), axis=(1, 2))
    )
    faces = faces[valid_faces]
    ...
    def save_surface(points: np.ndarray, path, *, face_values=None, cmap="viridis", title: str, surface_faces=None) -> None:
        render_faces = faces if surface_faces is None else surface_faces
        fig = plt.figure(figsize=(9, 7), dpi=160)
        ax = fig.add_subplot(111, projection="3d")
        triangles = points[render_faces]
        if face_values is None:
            face_colors = np.tile(np.asarray([[0.16, 0.48, 0.76, 0.95]]), (len(render_faces), 1))
        else:
            ...
            if upper <= lower:
                upper = lower + 1.0e-12
            normalizer = Normalize(vmin=lower, vmax=upper, clip=True)
            mapper = plt.cm.ScalarMappable(norm=normalizer, cmap=cmap)
            face_colors = mapper.to_rgba(face_values)
            fig.colorbar(mapper, ax=ax, shrink=0.7, pad=0.1)
        collection = Poly3DCollection(triangles, facecolors=face_colors, edgecolors="none", linewidths=0.0)
        ax.add_collection3d(collection)
        finite_points = points[np.unique(render_faces)]
        limits = np.ptp(finite_points, axis=0)
        center = np.mean(finite_points, axis=0)
        radius = max(float(np.max(limits)) * 0.55, 1.0e-12)
    ...
    save_surface(reference, output_dir / "stitched_reference_scene.png", title="Stitched reference surface", surface_faces=all_faces)
    save_surface(deformed, output_dir / "stitched_deformed_scene.png", title="Stitched deformed surface", surface_faces=all_faces)
    face_displacement = displacement[all_faces].mean(axis=1)
    face_magnitude = np.linalg.norm(face_displacement, axis=1)
    save_surface(
        reference,
        output_dir / "stitched_displacement_umag.png",
        face_values=face_magnitude,
        title="Stitched displacement magnitude",
        surface_faces=all_faces,
    )
    for component, name in enumerate(("ux", "uy", "uz")):
        save_surface(
            reference,
            output_dir / f"stitched_displacement_{name}.png",
            face_values=face_displacement[:, component],
            cmap="coolwarm",
            title=f"Stitched displacement {name.upper()}",
            surface_faces=all_faces,
        )
